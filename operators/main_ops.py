...
class SEQUENCER_OT_generate_movie(Operator):
    ...
    def execute(self, context):  # noqa: C901
        ...
        # Detect mode
        has_video = bool(getattr(scene, "movie_path", "")) and input_mode == "input_strips"
        ...
        for i in range(scene.movie_num_batch):
            ...
            if has_video and input_mode == "input_strips":
                ...
                for area in screen.areas:
                    if area.type == "SEQUENCE_EDITOR":
                        ...
                        with _ctx.temp_override(window=window, area=area):
                            ...
                            scene.sequence_editor.active_strip = scene.sequence_editor.strips[-1]
                            if i > 0:
                                scene.frame_current = scene.sequence_editor.active_strip.frame_final_start
    ...
    def execute(self, context):  # noqa: C901
        ...
        if do_inpaint:
            ...
            if not mask_strip:
                ...
                return {"CANCELLED"}
            if mask_strip.type in ("MASK", "COLOR", "SCENE", "META"):
                mask_strip = get_render_strip(self, context, mask_strip)
            mask_image = load_first_frame(get_strip_path(mask_strip))
            if not mask_image:
            ...
            if not init_image:
                self.report({"INFO"}, "Failed to load init image for inpaint.")
                return {"CANCELLED"}
